...
function MemoryMatching({ animePool, level, onChangeLevel }) {
    ...
    function startNewGame() {
        const newCards = createMatchingCards(animePool, level);
    ...
    function handleCardClick(card) {
        if (isChecking || flippedCards.includes(card.id) || matchedCards.includes(card.id)) {
            return;
        }
    ...
    return (
        <div className={styles.game}>
            <header className={styles.header}>
                <p className={styles.eyebrow}>🧩 MEMORY MATCHING</p>
            ...
            <div className={styles.gameInfo}>
                ...
                <div className={styles.stat}>
                    <span>Pairs</span>
                    <strong>{matchedCards.length / 2}</strong>
                </div>
            </div>

            <div className={styles.actions}>
                <button type="button" onClick={startNewGame}>New Game</button>
                <button type="button" onClick={onChangeLevel}>Change Level</button>
            </div>

            {gameWon && (
                <div className={styles.result}>
                    <h2>🎉 You Win!</h2>
                    <p>You found all {cards.length / 2} pairs!</p>
